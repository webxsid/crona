...
func Resolve(action dialogstate.Action, state State, deps Deps) tea.Cmd {
	...
	r.Register(
		"create_repo",
		func(action dialogstate.Action) tea.Cmd { return deps.CreateRepo(action.Name, action.Description) },
	)
	...
	r.Register("create_habit", func(action dialogstate.Action) tea.Cmd {
		return deps.CreateHabitWithPath(
			action.RepoName,
			action.StreamName,
			action.Name,
			action.Description,
	...
	r.Register("create_issue_default", func(action dialogstate.Action) tea.Cmd {
		return deps.CreateIssueWithPath(
			action.RepoName,
			action.StreamName,
			action.Title,
			action.Description,
	...
	r.Register(
		"create_checkin",
		func(action dialogstate.Action) tea.Cmd { return checkinCmd(action, deps) },
	)
	r.Register(
		"edit_checkin",
		func(action dialogstate.Action) tea.Cmd { return checkinCmd(action, deps) },
	...
	r.Register(
		"export_report",
		func(action dialogstate.Action) tea.Cmd { return exportCmd(action, state, deps) },
	)
	r.Register(
		"set_export_reports_dir",
		func(action dialogstate.Action) tea.Cmd { return deps.SetExportReportsDir(action.Path) },
	)
	r.Register(
		"set_export_ics_dir",
		func(action dialogstate.Action) tea.Cmd { return deps.SetExportICSDir(action.Path) },
	)
	r.Register(
		"patch_setting",
	...
	r.Register("patch_telemetry_settings", func(action dialogstate.Action) tea.Cmd {
		...
		return deps.PatchTelemetrySettings(
			action.UsageTelemetry,
			action.ErrorReporting,
			action.RestartAfterSave,
		)
	})
	r.Register("complete_onboarding", func(action dialogstate.Action) tea.Cmd {
		...
		return deps.CompleteOnboarding(
			action.UsageTelemetry,
			action.ErrorReporting,
			action.RestartAfterSave,
		)
	...
	r.Register(
		"patch_rest_protection",
		func(action dialogstate.Action) tea.Cmd { return patchRestProtectionCmd(action, state, deps) },
	)
	r.Register(
		"delete",
	...
	r.Register("extend_hard_limit", func(action dialogstate.Action) tea.Cmd {
		if action.TimerExtend == nil {
			return deps.ErrorCmd(errors.New("timer extend payload is missing"))
		}
		return deps.ExtendHardLimit(*action.TimerExtend)
	})
	...
	r.Register(
		...
		func(action dialogstate.Action) tea.Cmd { return deps.OpenExternalPath(action.Path) },
	)
...
func deleteCmd(action dialogstate.Action, state State, deps Deps) tea.Cmd {
	...
	r.Register(
		"repo",
		func(action dialogstate.Action) tea.Cmd { return deps.DeleteRepo(dialogstate.ParseNumericID(action.ID)) },
	)
	...
	r.Register(
		"checkin",
		func(action dialogstate.Action) tea.Cmd { return deps.DeleteDailyCheckIn(action.ID) },
	)
