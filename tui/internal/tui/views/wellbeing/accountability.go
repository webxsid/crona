...
func wellbeingAccountabilityLines(theme types.Theme, state types.ContentState) []string {
	...
	if planned == 0 && completed == 0 && failed == 0 && abandoned == 0 && pending == 0 &&
		delayed == 0 &&
		score == 0 {
		return nil
	}
	lines := []string{
		"",
		fmt.Sprintf(
			"%s  planned %d   completed %d   failed %d",
			theme.StyleHeader.Render("Accountability"),
			planned,
			completed,
			failed,
		),
		theme.StyleDim.Render(
			fmt.Sprintf("pending rollback %d   abandoned %d", pending, abandoned),
		),
	}
	if score > 0 || delayed > 0 || highRisk > 0 {
		lines = append(
			...
			theme.StyleDim.Render(
				fmt.Sprintf(
					"score %.1f   backlog %.1f   delayed %d   high risk %d",
					score,
					pressure,
					delayed,
					highRisk,
...
func wellbeingCompactAccountabilityLines(theme types.Theme, state types.ContentState) []string {
	...
	return []string{
		fmt.Sprintf(
			"%s  Planned %d  Completed %d  Failed %d  Pending %d",
			theme.StyleHeader.Render("Accountability"),
			planned,
			completed,
			failed,
