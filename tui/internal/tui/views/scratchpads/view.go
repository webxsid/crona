...
func renderView(theme types.Theme, state types.ContentState) string {
	if !state.ScratchpadOpen {
		return viewchrome.RenderScratchpadPlaceholder(theme, state)
	}
	...
	lines := []string{
		...
		theme.StyleHeader.Render(viewhelpers.Truncate(state.ScratchpadName, innerW)),
		theme.StyleDim.Render(viewhelpers.Truncate(state.ScratchpadPath, innerW)),
	}
