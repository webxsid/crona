...
func newView() viewui.View {
	return viewui.View{
		Panes: []viewui.Pane{
			viewui.NewRenderPane("ops", true, renderView),
		},
...
func renderOpRow(
	...
	row := fmt.Sprintf(
		...
		ts,
		entityW,
		viewhelpers.Truncate(string(op.Entity), entityW),
		actionW,
		viewhelpers.Truncate(string(op.Action), actionW),
		viewhelpers.Truncate(target, targetW),
	)
	switch {
	case i == cur && active:
		return theme.StyleCursor.Render(
			viewchrome.SelectionCursor + " " + viewhelpers.Truncate(row[2:], state.Width-6),
		)
	case i == cur:
...
func currentOpsLimit(state types.ContentState) int {
	...
	visibleRows = max(visibleRows, 10)
	return visibleRows
}
